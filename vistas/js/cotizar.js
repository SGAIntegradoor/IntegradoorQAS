...
async function cotizarOfertas() {
  ...
  if (
      ...
      if (!primerIntentoRealizado) {
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            // ====== Separar promesas prioritarias y Bolivar ======
            let cont = [];
            let contBolivar = [];
            // ========================================================
            ...
            const mostrarAlertarCotizacionFallida = (aseguradora, mensaje) => {
              ...
              if (filaExistente) {
                // Acceder directamente a las celdas existentes
                const celdaContador = filaExistente.cells[2]; // 'Productos cotizados'
                const celdaCotizo = filaExistente.cells[1]; // Icono de estado
                const celdaResponse = filaExistente.cells[3]; // 'Observaciones'

                // Si la cotización no fue exitosa, actualiza mensaje e icono
    ...
    `;
            ...
            //console.log(cont)
            // Primero resolver las promesas principales, luego las de Bolívar
            Promise.all(cont).then(() => {
              // Después de las principales, ejecutar Bolívar
              Promise.all(contBolivar).then(() => {
                // $("#btnCotizar").hide();
                $("#loaderOferta").html("");
                //$("#loaderOfertaBox").css("display", "none");
            ...
            }); // Cierre de Promise.all(contBolivar)
            }); // Cierre de Promise.all(cont)
          },
        });
        countOfferts();
        } // Cierre del if (!primerIntentoRealizado)
      else {
        //ZONA RECOTIZACIÓN//
        $("#loaderRecotOferta").html(
        ...
        //  console.log(cont)
        // Primero resolver promesas principales, luego Bolívar
        Promise.all(cont).then(() => {
          Promise.all(contBolivar).then(() => {
            $("#loaderOferta").html("");
            $("#loaderRecotOferta").html("");
          ...
          }); // Cierre de Promise.all(contBolivar) en recotización
        }); // Cierre de Promise.all(cont) en recotización
      }
