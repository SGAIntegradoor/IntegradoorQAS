...
const construirHtmlContinuidad = (continuidades) => {
  ...
  continuidades.forEach((continuidad) => {
    if (continuidad !== "")
      html += `<li><span class="text-config">✅ ${continuidad}</span></li>`;
  });
  html += "</ul>";
...
const construirHtmlCambioIntermediario = (cambioPoliticas) => {
  ...
  cambioPoliticas.forEach((cambioPolitica) => {
    if (cambioPolitica !== "")
      html += `<li><span class="text-config">✅ ${cambioPolitica}</span></li>`;
  });
  html += "</ul>";
...
const editarAyudaVenta = async () => {
  ...
  formData.append(
    "linea_de_atencion",
    d.querySelector("#linea_atencion").value
  );
...
const editarAyudaVentaRequest = async (_formData) => {
  const req = await fetch(
    "./vistas/modulos/AyudaVentas/AyudaVentasController.php",
    {
      method: "POST",
      headers: {
  ...
  await fetch("./vistas/modulos/AyudaVentas/AyudaVentasController.php", {
    ...
    .then((data) => {
      ...
      data.forEach((ayudaVenta) => {
        ...
        const centrosDeInspeccion =
          ayudaVenta.centro_de_inspeccion != null
            ? ayudaVenta.centro_de_inspeccion.split("@")
            : [];
        const continuidades =
          ayudaVenta.continuidad != null
            ? ayudaVenta.continuidad.split("-")
            : [];
        const cambioPoliticas =
          ayudaVenta.politicas_cambio_intermediario != null
            ? ayudaVenta.politicas_cambio_intermediario.split("-")
            : [];
        let formasDePago = [];
        if (rol == "x") {
          formasDePago =
            ayudaVenta.formas_de_pago_freelance != null
              ? ayudaVenta.formas_de_pago_freelance.split("@")
              : [];
        } else {
          formasDePago =
            ayudaVenta.formas_de_pago != null
              ? ayudaVenta.formas_de_pago.split("@")
              : [];
        }
        let partTemplate = `
                <tr >
                    <td style="text-align: center;"><ul style="margin-top: 60px;"><img src="./vistas/modulos/AyudaVentas/src/logos/${
                      ayudaVenta.aseguradora
                    }.png" class="img-responsive" width="80" style="margin: 0 auto; height: ${
          ayudaVenta.aseguradora === "Solidaria" ? "40px" : "auto"
        };"></ul></td>
                    <td><ul style="margin-top: 60px; text-align: center;">${
                      ayudaVenta.linea_de_atencion
                    }</ul></td>`;
        if (ayudaVenta.link_clausulado) {
          partTemplate += `<td style="text-align: center;"><ul style="margin-top: 60px;"><button class="btn btn-alert" style="border-color: #88d600; width: 90%; color: #88d600; font-weight: 500;" onclick="validarPermisoClausulado('${
            ayudaVenta.link_clausulado
          }')">${ayudaVenta.link_clausulado.substring(
            0,
            24
          )}</button></ul></td>`;
        } else {
        ...
        if (ayudaVenta.path_sarlaft || ayudaVenta.path_sarlaft2) {
          if (
            ayudaVenta.aseguradora == "Allianz" ||
            ayudaVenta.aseguradora == "Equidad" ||
            ayudaVenta.aseguradora == "Previsora"
          ) {
            ...
            let sarlaftButtons =
              '<td><ul style="margin-top: 60px; display: flex; flex-direction: column;">';
            sarlaftButtons += ayudaVenta.path_sarlaft
              ? `<a href="${ayudaVenta.path_sarlaft}" class="btn btn-alert" style="background: red; color: #fff; font-weight: 500; margin-bottom: 5px;" target="_blank">PDF PN</a>`
              : '<button class="btn btn-alert" style="background: red; color: #fff; font-weight: 500; margin-bottom: 15px;">PDF PN</button>';
            sarlaftButtons += ayudaVenta.path_sarlaft2
              ? `<a href="${ayudaVenta.path_sarlaft2}" class="btn btn-alert" style="background: red; color: #fff; font-weight: 500;" target="_blank">PDF PJ</a>`
              : '<button class="btn btn-alert" style="background: red; color: #fff; font-weight: 500;">PDF PJ</button>';
        ...
        partTemplate += `
            ...
            <style>
                ...
                <td class="fixed-width">${construirHtmlCentrosDeInspeccion(
                  centrosDeInspeccion
                )}</td>
                <td class="fixed-width continuidad">${construirHtmlContinuidad(
                  continuidades
                )}</td>
                <td class="fixed-width cambio-intermediario">${construirHtmlCambioIntermediario(
                  cambioPoliticas
                )}</td>
                <td class="fixed-width">${construirHtmlFormasDePago(
                  formasDePago
                )}</td>`;
...
let centros = [];
document
  .querySelector("#agregarCentroDeInspeccion")
...
const editarFormaDePago = (_index, _forma_de_pago_value_index) => {
  const d = document;
  formasDePago[_index] = d.querySelector(
    "#" + _forma_de_pago_value_index
  ).value;
};
